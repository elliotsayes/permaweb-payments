  ...
  init(env) {
    ...
    return {
      ...
      isLicensed(contract, addr) {
        return of({ contract, addr }).chain(getLicenseInfo).chain(findInteractionsByAddress).chain(getValidity).map(allValid).toPromise();
      },
    ...
    function findInteractionsByAddress(ctx) {
      ...
      return query(buildInteractionsQuery(), { addrs: [ctx.addr], contracts: [ctx.contract] }).map(
        ...
        (interactions) => interactions.length > 0 ? Resolved({ ...ctx, interactions }) : Rejected({ ok: false, message: "No Interactions Found" })
      );
    }
